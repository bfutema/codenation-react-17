...
const Player = () => {
  ...
  const togglePlayPause = () => {
    ...
    if (isPlaying && !audioPlayer.paused) setIsPlaying(false);
    else if (!isPlaying && audioPlayer.paused) setIsPlaying(true);
  };
  ...
  useEffect(() => {
    if (playingNowId === prevPlayingNowId) return;
  ...
  useEffect(() => {
    ...
    if (prevPlayingNowId === playingNowId) {
      if (isPlaying && audioPlayer?.paused) audioPlayer.play();

      if (!isPlaying && !audioPlayer?.paused) audioPlayer.pause();
    } else {
      setIsPlaying(true);
    }
  ...
  useEffect(() => {
    if (playerHeight > 0) dispatch(setPlayerHeight(playerHeight));
  }, [playerHeight, dispatch, isPlaying]);
